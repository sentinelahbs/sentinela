...
def get_camera_calibration(
    ...
    """Usado pela BOX (calibration_sync.py) a cada ciclo — mesma
    autenticação por X-API-Key da loja que neighbor-ids já usa. Campos
    None significam 'sem calibração remota', a box mantém o default
    local (ver config.py)."""
    camera = db.query(Camera).filter(Camera.id == camera_id, Camera.store_id == store_id, Camera.active.is_(True)).first()
    if camera is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Câmera não encontrada nesta loja")

    return CameraCalibrationOut(
        camera_id=camera.id,
        zone_of_interest=camera.zone_of_interest,
        hand_still_frames_threshold=camera.hand_still_frames_threshold,
